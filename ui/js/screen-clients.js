...
const ClientsScreen = {
        ...
        return `<a href="#client/${p.id}" class="cl-card">
            <div class="flex-row">
                <div style="flex:1;min-width:0">
                    ...
                    <div class="cl-sub"><span class="flex-row" style="gap:5px">${UI.crest(club)}${club ? club.name : 'Free agent'}</span><span style="color:var(--text-chevron)">·</span><span>${roleLabel(p.squadRole, p.age)}</span></div>
                </div>
    ...
    pickSort() {
        Router.sheet(`<div class="sheet__handle"></div><div class="sheet__title">Sort clients</div>
            ${this.SORTS.map(([id, label]) => `<button class="list-row" style="width:100%;background:none;border:0;cursor:pointer;text-align:left" onclick="ClientsScreen.setSort('${id}')"><span style="flex:1;color:var(--text)">${label}</span>${this.state.sort === id ? `<i class="ti ${this.state.dir === 'asc' ? 'ti-sort-ascending' : 'ti-sort-descending'}" style="color:var(--accent)"></i>` : ''}</button>`).join('')}`);
    },
    setSort(id) {
...
const ClientHistory = {
    ...
    render(el) {
        const rows = this.rows().sort((a, b) => (a[this.state.sort] - b[this.state.sort]) * (this.state.dir === 'asc' ? 1 : -1));
        ...
        el.innerHTML = `<p class="hint" style="margin-bottom:var(--space-4)">Every player who has been your client — current, released or retired.</p>
        <div class="chip-row" style="margin-bottom:var(--space-4)">${posBtns}</div>
        ${rows.length ? rows.map(r => `<a href="#client/${r.p.id}" class="list-row" style="cursor:pointer">
            <div style="flex:1;min-width:0"><div class="row-title">${UI.flag(r.p.nationality)} ${r.p.name}</div><div class="row-sub">${r.p.position} · ${r.seasons} season(s) · <span class="pill" style="padding:1px 7px;font-size:10.5px">${r.status}</span></div></div>
            <div style="text-align:right;font-size:12px;color:var(--text-muted)">${r.apps} apps · ${r.p.position === 'GK' ? r.cs + ' cs' : r.goals + ' g'}<br>${UI.ratingText(r.avg)}</div>
        </a>`).join('') : '<div class="empty"><div class="empty__icon"><i class="ti ti-history"></i></div><div class="empty__title">No history yet</div><div class="empty__hint">Sign players and their careers will be recorded here.</div></div>'}`;
    },
