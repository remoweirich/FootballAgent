...
const ClubScreen = {
    ...
    render(el, clubId) {
        ...
        if (!c) {
            ...
            el.innerHTML = `<div class="empty">${I18n.t('club.unknown')}</div>`; return;
        }
        ...
        const honours = Object.keys(titleCount).length
            ...
            : `<p class="muted">${I18n.t('club.noHonours')}</p>`;
        ...
        const finishes = hist.length ? hist.slice().reverse().map(h => `<div class="frow"><span class="frow__k">${GameState.seasonLabelFor(h.year)} · ${(COMPETITIONS[h.division] || {}).short || h.division}</span><span class="frow__v">${h.position === 1 ? '🥇 ' : ''}${UI.ordinal(h.position)}${(h.trophies || []).length ? ' · 🏆 ' + h.trophies.map(t => (COMPETITIONS[t] || {}).short || t).join(', ') : ''}</span></div>`).join('')
            : `<p class="muted">${I18n.t('club.noSeasons')}</p>`;
        ...
        ${euBestHTML}
        <a class="gbtn" style="margin-top:var(--space-3)" href="${Router.link('comphist', c.division)}"><i class="ti ti-history"></i>${I18n.t('club.divHistory', { div: c.divisionName })}</a>
        <div class="section-label">${I18n.t('club.yourClients')}</div>
        <div class="chip-row" style="margin:var(--space-2) 0 var(--space-3)">
            <button class="htog ${mode === 'season' ? 'is-on' : ''}" onclick="ClubScreen.setMode('${clubId}','season')">${I18n.t('club.currentSeason')}</button>
            <button class="htog ${mode === 'all' ? 'is-on' : ''}" onclick="ClubScreen.setMode('${clubId}','all')">${I18n.t('club.allTime')}</button>
        </div>
        <div class="chip-row" style="margin:0 0 var(--space-3)">
            ${['apps', 'goals', 'assists', 'avg'].map(k => `<button class="htog ${this.state.sort === k ? 'is-on' : ''}" onclick="ClubScreen.setSort('${clubId}','${k}')">${I18n.t({ apps: 'club.apps', goals: 'club.goals', assists: 'club.assists', avg: 'club.rating' }[k])}</button>`).join('')}
        </div>
    ...
    europeBestHTML(clubId) {
        ...
        if (!rows) return '';
        return `<div class="section-label">${I18n.t('club.bestEurope')}</div><div class="fcard" style="margin-top:var(--space-2)">${rows}</div>`;
    },
    setSort(clubId, k) { this.state.sort = k; Router.refresh(); },
    setMode(clubId, m) { this.state.mode = m; Router.refresh(); }
};
Router.register('clubs', { isMain: false, parent: 'leagues', title: () => I18n.t('club.title'), render(el, params) { ClubScreen.render(el, params[0]); } });
